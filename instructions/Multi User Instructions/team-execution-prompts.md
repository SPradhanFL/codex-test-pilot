...
Every execution applies a 120-second failure-observation window: before a scenario can be marked **FAIL**, the runner must poll the missing expected page, element, navigation, or state for the full 120 seconds, then capture final evidence and record the timeout. Transient overlays and missing page/account shells also receive up to 120 seconds, with one safe refresh after 60 seconds, before **BLOCKED** is used. Genuine missing-data, permission, safety, and unsupported prerequisites keep their original classifications.
