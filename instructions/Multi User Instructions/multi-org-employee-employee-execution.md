...
4. For each Employee organization context, in visible order:
   ...
   3. Execute Employee scenario **14**. Prefer an existing absence read-only; if none is available, execute the temporary self-service create-and-cleanup fallback in `tests/navigation/absence-tab.md` within this exact organization context.
   4. Execute Employee scenario **16** from a fresh login with that exact organization context reselected.
5. A missing absence triggers Scenario 14's fallback. Mark it **BLOCKED** only when safe creation, unique reopening, or verified cleanup is unavailable; continue to scenario 16 and remaining contexts.
