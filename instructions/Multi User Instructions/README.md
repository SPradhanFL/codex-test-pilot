...
15. Before any controller finalizes a **FAIL** caused by missing, slow, or incomplete UI, apply the shared 120-second recovery policy in `instructions/project-instructions.md`. Measure every screen/control load, record `SLOW_UI_LOAD` with the actual elapsed seconds above 30 seconds, capture evidence at or after timeout, and state the 120-second expiration in the failed workflow. A Sidekick that recovers within 120 seconds under HCMAT-79933 remains PASS when all assertions succeed and includes the Jira ticket in the warning description. Do not delay definite BLOCKED or NOT TESTED prerequisites. Label exact `config/known-failures.json` matches as known failures while keeping canonical status FAIL.
