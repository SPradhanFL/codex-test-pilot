...
Before reporting, execute controller-level scenarios **43, 44, 45, and 46** once for this combination account across every required Employee and Substitute organization context. Verify both organization data isolation and destination-role validity, preserve the selected organization/role through the TA -> AM -> TA round-trip, and execute scenario 46 last. Do not duplicate these four results inside the per-context blocks.

Follow `instructions/html-reporting-standard.md`. Create the role report under `reports/full-suite/<OrgId>/<YYYYMMDD-HHMMSS>/roles/multi-org-employee-substitute/`. Group results by organization and role, with all four assigned outcomes for every context plus four controller-level context outcomes. Report organization labels only to the extent needed to prove context separation; omit personal and secret data. Include screenshots, one continuous video, expected/actual results, and failure reproduction steps.
