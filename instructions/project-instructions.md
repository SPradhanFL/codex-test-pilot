...
- If the project-scoped disposable browser cannot be opened or discovered, make at least **three retry attempts after the initial failure** (four total attempts) before marking the run **BLOCKED**. Before each retry, recheck that the project Playwright MCP backend is available and call its browser-close operation when callable so a partial session is not reused. Record each attempt number and its sanitized result in the report. If any attempt succeeds, continue in that fresh isolated browser. Never satisfy this retry rule by opening a connected Chrome-extension profile, persistent user-data directory, saved storage state, or another non-isolated browser.
