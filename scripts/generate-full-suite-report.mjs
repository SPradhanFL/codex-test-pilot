...
const redactSensitiveSource = (source) => source.replace(
  /(\b(?:password|phone(?: number)?|pin)\b[^`\r\n]{0,40})`[^`\r\n]+`/gi,
  '$1`[REDACTED]`'
);
...
for (const item of executions) {
  ...
  const source = redactSensitiveSource(fs.readFileSync(sourcePath, 'utf8'));
  const failures = item.reproduce?.length ? `<section class="panel failure"><h2>${esc(item.status)} scenario and steps to reproduce</h2><ol>${item.reproduce.map((step) => `<li>${esc(step)}</li>`).join('')}</ol><p><strong>Expected:</strong> ${esc(item.expected ?? 'The documented scenario completes successfully.')}</p><p><strong>Actual:</strong> ${esc(item.summary)}</p></section>` : '';
...
const failureSections = executions.filter((item) => item.status === 'FAIL' || item.status === 'BLOCKED').map((item) => `<article class="panel failure"><h3>${item.execution} · ${esc(item.scenario)}</h3><p>${esc(item.summary)}</p><ol>${(item.reproduce ?? []).map((step) => `<li>${esc(step)}</li>`).join('')}</ol><p><strong>Expected:</strong> ${esc(item.expected ?? 'The documented scenario completes successfully.')}</p><a href="scenarios/${esc(item.slug)}.html">Open full execution report</a></article>`).join('');
...
<section><h2>Failed and blocked scenarios</h2>${failureSections}</section>
