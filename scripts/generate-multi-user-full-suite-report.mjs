...
const evidenceIntegrityPath = path.join(runDir, 'evidence-integrity.json');
const evidenceIntegrity = fs.existsSync(evidenceIntegrityPath)
  ? readJson(evidenceIntegrityPath)
  : {status: 'VALID', summary: 'No evidence-integrity exception was recorded.'};
...
if (!['VALID', 'INVALID'].includes(evidenceIntegrity.status)) throw new Error('evidence-integrity.json has an invalid status.');
if (evidenceIntegrity.status === 'INVALID' && !String(evidenceIntegrity.summary ?? '').trim()) {
  throw new Error('Invalid evidence integrity must include a summary.');
}
const evidenceIntegrityNotice = evidenceIntegrity.status === 'INVALID'
  ? `<section class="panel warning-note"><div class="card-top"><h2>Evidence integrity warning</h2><span class="badge fail">INVALID</span></div><p>${esc(evidenceIntegrity.summary)}</p><p><strong>Impact:</strong> ${esc(evidenceIntegrity.impact ?? 'Do not use the recorded video or screenshots as visual proof. Functional status totals are retained from live browser DOM validation.')}</p></section>`
  : '';
...
const normalizedAccounts = data.accounts.map((account, accountIndex) => {
  ...
  const workflows = rawWorkflows.map((workflow, workflowIndex) => {
    ...
    return {...workflow, slug, startSeconds, endSeconds, steps, screenshots, warnings};
  });
...
const shell = (title, body) => `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><style>${css}</style></head><body>${evidenceIntegrityNotice}${body}<script>${videoScript}</script></body></html>`;
